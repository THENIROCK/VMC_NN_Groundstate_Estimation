...
# ╔═╡ 1cbaab44-1da7-11f0-267e-e5531c2c59fd
module DenseNet

using Flux
using Flux: Chain, glorot_uniform
using Functors

export get_dense, ConditionalDenseNetwork

"""
Create a fully connected network (Chain) given layer dimensions.
- dims: vector of layer sizes (input, hidden..., output)
- zero_initialized: if true, initialize final layer weights and biases to zeros
"""
function get_dense(dims::Vector{Int}; zero_initialized::Bool=false)
    layers = []
    for i in 1:length(dims)-1
        in_dim = dims[i]
        out_dim = dims[i+1]
        # Choose initializer for final layer if zero_initialized
        initW = (zero_initialized && i == length(dims)-1) ? zeros : glorot_uniform
        initb = zeros
        # Explicitly reference Flux.Dense to avoid naming conflicts
        push!(layers, Flux.Dense(in_dim, out_dim, initW, initb))
        if i < length(dims)-1
...
"""
Conditional Dense Network:
Given a conditioning vector of size dim_cond,
it outputs weights and biases for each layer, then applies them to input x.
- dims: vector of layer sizes
- dim_cond: size of conditioning input
"""
struct ConditionalDenseNetwork
    dims::Vector{Int}
    dim_cond::Int
    denses::Vector{Chain}
    biases::Vector{Chain}
...
Functors.@functor ConditionalDenseNetwork

function ConditionalDenseNetwork(dims::Vector{Int}, dim_cond::Int)
    denses = [Chain(Flux.Dense(dim_cond, dims[i] * dims[i+1])) for i in 1:length(dims)-1]
    biases = [Chain(Flux.Dense(dim_cond, dims[i+1])) for i in 1:length(dims)-1]
    return ConditionalDenseNetwork(dims, dim_cond, denses, biases)
end

function (m::ConditionalDenseNetwork)(x::AbstractMatrix{<:Real}, cond::AbstractMatrix{<:Real})
    batch = size(x, 1)
    @assert batch == size(cond, 1) "Batch size mismatch"
    @assert size(x, 2) == m.dims[1] "Input feature dimension mismatch"
    @assert size(cond, 2) == m.dim_cond "Condition dimension mismatch"
    y = x
    for i in 1:length(m.denses)
        d_in = m.dims[i]
        d_out = m.dims[i+1]
        # produce flattened weights and biases per example
        Wflat = m.denses[i](cond)           # (batch, d_in*d_out)
        b = m.biases[i](cond)               # (batch, d_out)
        # reshape into (batch, d_out, d_in)
        W = reshape(Wflat, batch, d_out, d_in)
        # apply each batch entry
        Y = Matrix{Float32}(undef, batch, d_out)
        for j in 1:batch
            Y[j, :] = W[j, :, :] * y[j, :] .+ b[j, :]
        end
        # activation except last layer
        y = i < length(m.denses) ? tanh.(Y) : Y
    end
    return y
end
...
manifest_format = "2.0"
project_hash = "4f85904f4360f705315cd878df4b4e30187f44d3"
