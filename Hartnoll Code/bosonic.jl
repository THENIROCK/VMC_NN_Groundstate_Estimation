...
# ╔═╡ 4675ca54-1d93-11f0-2a51-7f25b0ed5497
begin
	# bosonic.jl
	...
	using .Algebra: SU, SO2, MatrixModel
	using .Wavefunc: BlockAutoregressiveWavefunction
	using .Sampler: BlockAutoregressiveSampler, sample
	using .Train: minimize, evaluate, gauge_evaluate
	using .Obs: casimir, minimal_BMN_energy, group_action
	using Flux: Adam
	...
	# build algebra, wavefunction & sampler
	algebra = MatrixModel(SU(N), 2, 0)
	wavefunc = BlockAutoregressiveWavefunction(algebra.dim_b, algebra.dim_f; alpha=α)
	...
	rotation = group_action(algebra, SO2(), zeros(Float64, batch_size, 1))
	obs = Dict("H" => hamil,
	           "G" => gauge,
	           "R" => rotation)
	...
	filename = "results/bosonic/N=$(N),l=$(round(ℓ; digits=4)),c=$(round(c; digits=2)),alpha=$(α)"
	println(filename)
	...
	for (name, op) in obs
	    ...
	    evaluate(wavefunc, sampler, op, 1_000_000; normalized=true, filename=filename)
	
	    println("Gauge evaluating $name")
	    gauge_evaluate(wavefunc, sampler, op, 1_000_000,
	                   algebra, SU(N); normalized=true, filename=filename)
	end
end
...
manifest_format = "2.0"
project_hash = "e7920bdf80285454a063a1409778cdc09f5fcf1a"
