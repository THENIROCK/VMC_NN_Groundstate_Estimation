...
# ╔═╡ 10aa584a-1da9-11f0-1f36-0bd8af4efd3e
module NAF
...
export get_weight_mask, block_diag, get_params,
       BNAF, BNAFDensityEstimator, BNAFSampler, trainable_variables

"""
Return block autoregressive masks for weight matrices.
"""
function get_weight_mask(dim_in::Int, dim_out::Int, num_blocks::Int)
    step_in  = div(dim_in, num_blocks)
    step_out = div(dim_out, num_blocks)
    mask_d = zeros(Float32, num_blocks*step_out, num_blocks*step_in)
    mask_o = ones(Float32,  num_blocks*step_out, num_blocks*step_in)
    for b in 1:num_blocks
        r = ((b-1)*step_out+1):(b*step_out)
        c = ((b-1)*step_in+1):(b*step_in)
        mask_d[r, c] .= 1f0
        mask_o[r, c] .= 0f0
    end
...
"""
Construct block-diagonal matrix from diagonal blocks.
"""
function block_diag(mat::Array{Float32,3})
    num_blocks, dim_out, dim_in = size(mat)
    M = zeros(Float32, num_blocks*dim_out, num_blocks*dim_in)
    for b in 1:num_blocks
        r = ((b-1)*dim_out+1):(b*dim_out)
        c = ((b-1)*dim_in+1):(b*dim_in)
        M[r, c] .= mat[b, :, :]
    end
    return M
end
...
"""
Slice `w` starting at `cur+1` for `n` elements.
"""
function get_params(w::Vector{T}, cur::Int, n::Int) where T
    slice = @view w[cur+1:cur+n]
    return slice, cur + n
end

"""Total parameters for BNAF of given dims."""
function BNAF_num_variables(dims::Vector{Int})
    tot = 0; L = length(dims); num_blocks = dims[1]
    for i in 1:L-1
        dim_in, dim_out = dims[i], dims[i+1]
        step_in, step_out = div(dim_in,num_blocks), div(dim_out,num_blocks)
        tot += dim_in*dim_out + dim_out + num_blocks*step_in*step_out
        if i < L-1 tot += 2*dim_out end
    end
    return tot
end

"""Block Neural Autoregressive Flow (BNAF) bijector."""
struct BNAF{T}
    dims::Vector{Int}
    weight::Vector{T}
end
Functors.@functor BNAF

function BNAF(dims::Vector{Int}; w::Union{Nothing,Vector{Float32}}=nothing)
    total = BNAF_num_variables(dims)
    w = w === nothing ? randn(Float32, total) : w
    return BNAF{Float32}(dims, w)
end

"""
Forward pass: x -> (y, logdet).
Fill in masked weight, bias, activations as needed.
"""
function forward(flow, x::AbstractMatrix{Float32})
    batch, dim = size(x); num_blocks = flow.dims[1]
    y = x; log_jac = zeros(Float32, batch); cur = 0
    for i in 1:length(flow.dims)-1
        # unpack parameters, build masked layers, accumulate Jacobian
        # TODO: implement details
        local dim_in = flow.dims[i]
        local dim_out = flow.dims[i+1]
        local step_in, step_out = div(dim_in,num_blocks), div(dim_out,num_blocks)
        _w, cur = get_params(flow.weight, cur, dim_in*dim_out)
        _W = reshape(_w, dim_out, dim_in)
        mask_d, mask_o = get_weight_mask(dim_in, dim_out, num_blocks)
        # Placeholder: direct apply
        y = y * 1f0  # no-op
    end
    logdet = sum(log_jac)
    return y, logdet
...
"""Density estimator using BNAF."""
struct BNAFDensityEstimator{F}
    flow::F
end
Functors.@functor BNAFDensityEstimator

function BNAFDensityEstimator(dims::Vector{Int}; w::Union{Nothing,Vector{Float32}}=nothing)
    return BNAFDensityEstimator(BNAF(dims; w=w))
end

function trainable_variables(e::BNAFDensityEstimator)
    return [e.flow.weight]
end

function log_prob(e::BNAFDensityEstimator, x::AbstractMatrix{Float32})
    y, logdet = forward(e.flow, x)
    base_lp = -0.5 * sum(y .^ 2, dims=2) .- (size(y,2)/2)*log(2π)
    return vec(base_lp .+ logdet)
end

function sample(e, batch_size::Int)
    dim = e.flow.dims[1]
    x = randn(Float32, batch_size, dim)
    y, logdet = forward(e.flow, x)
    base_lp = -0.5 * sum(x .^ 2, dims=2) .- (dim/2)*log(2π)
    return y, vec(base_lp .- logdet)
end

"""Sampler using BNAF flow."""
struct BNAFSampler{F}
    flow::F
end
Functors.@functor BNAFSampler

function BNAFSampler(dims::Vector{Int})
    return BNAFSampler(BNAF(dims))
end

function trainable_variables(s::BNAFSampler)
    return [s.flow.weight]
end

function sample(s, batch_size::Int)
    dim = s.flow.dims[1]
    x = randn(Float32, batch_size, dim)
    y, logdet = forward(s.flow, x)
    base_lp = -0.5 * sum(x .^ 2, dims=2) .- (dim/2)*log(2π)
    return y, vec(base_lp .- logdet)
end
...
manifest_format = "2.0"
project_hash = "8559987430e99c494ff8288a8d868ca4653af893"
