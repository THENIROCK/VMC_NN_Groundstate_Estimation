...
# ╔═╡ 06b2fc42-222e-11f0-06d1-1f78b8c07082
module Sampler
...
export BlockAutoregressiveSampler, trainable_variables, sample

# Abstract sampler interface
abstract type AbstractSampler end

# Block autoregressive neural sampler
mutable struct BlockAutoregressiveSampler <: AbstractSampler
    dim_b::Int                     # number of bosonic dims
    dim_f::Int                     # number of fermionic dims
    bosonic_network                # NAF.BNAFSampler or Nothing
end

# Constructor
function BlockAutoregressiveSampler(bosonic_dim::Int, fermionic_dim::Int)
    return BlockAutoregressiveSampler(bosonic_dim, fermionic_dim, nothing)
end

# Return trainable variables (only from bosonic network)
function trainable_variables(s)
    isnothing(s.bosonic_network) && return []
    return NAF.trainable_variables(s.bosonic_network)
end

# Lazy init of bosonic autoregressive sampler
function _get_bosonic_autoregressive(s)
    if s.bosonic_network === nothing
        # dims: [dim_b, dim_b, dim_b]
        s.bosonic_network = NAF.BNAFSampler([s.dim_b, s.dim_b, s.dim_b])
    end
    return s.bosonic_network
end

# Sample method: returns sample or (sample, log_prob)
function sample(s, batch_size::Int; with_log_prob::Bool=false)
    # Bosonic sample
    bns = _get_bosonic_autoregressive(s)
    sample_b, log_prob = NAF.sample(bns, batch_size)
    # Ensure correct size: (batch_size, dim_b)
    # Fermionic sample: random Bernoulli(0.5)
    if s.dim_f > 0
        sample_f_bool = rand(Bool, batch_size, s.dim_f)
        sample_f = Float32.(sample_f_bool)
        # log-prob of each fermionic sample: -dim_f * log(2)
        f_lp = fill(-s.dim_f * log(2), batch_size)
        log_prob .= log_prob .+ f_lp
    else
        sample_f = zeros(Float32, batch_size, 0)
    end
    # Concatenate
    sample_all = hcat(sample_b, sample_f)
    return with_log_prob ? (sample_all, log_prob) : sample_all
end
