...
# ╔═╡ 985a44ed-2774-4259-bf28-2074948f7bc5
### Toy Variational Monte Carlo for Mini-BMN Bosonic Sector (N=2) in Pluto
begin
    using PlutoUI, Flux, Distributions, LinearAlgebra, Random, Plots, Functors, ForwardDiff
end

# ╔═╡ 3eb0446a-37c4-41ec-bf3a-0db7dde578ba
### Define Pauli matrices and classical generators
begin
    σx = [0 1; 1 0]
    σy = [0 -im; im 0]
    σz = [1 0; 0 -1]
...
### Hamiltonian for the bosonic mini‑BMN (mass deformation ν)
begin
    function HB(X1,X2,X3,ν)
    A = Matrix{ComplexF64}(X1)
    B = Matrix{ComplexF64}(X2)
    C = Matrix{ComplexF64}(X3)
    # build commutators as dense matrices
    c12 = A*B - B*A
    c23 = B*C - C*B
    c31 = C*A - A*C

    Vc = 0.25*(tr(c12*c12) + tr(c23*c23) + tr(c31*c31)) # (c)ommutators are anti-hermitian
    Vm = 0.5*ν^2*(tr(A*A) + tr(B*B) + tr(C*C))
    Vs = ν * imag(tr(A*B*C - A*C*B))

    return real(Vc + Vm + Vs)
	end
end
...
begin
    ...
    #       d = randn()                 # one diagonal fluctuation
        ...
        # compute gradient and Hessian of f at x0
        g = ForwardDiff.gradient(f, x0)
        H = ForwardDiff.hessian(f,   x0)

        lap   = H[1,1] + H[2,2] + H[3,3]
        gradsq = g[1]^2 + g[2]^2 + g[3]^2

        return -0.5 * (lap + gradsq)
    end
end


# ╔═╡ ffb3928b-360a-46c3-855a-b51971b3a480
### Monte Carlo estimate of ⟨H⟩_ans
begin
    function energy(ans::Ansatz, ν; nsamples=1000)
        s = 0.0
        for i in 1:nsamples
            
			# generate normally distributed random number
			z1, z2, z3, z4= randn(), randn(), randn(), randn()

			# build X1 and X2 using the ansatz
			a = ans.r0 + ans.σ * z1
			b = ans.r0 + ans.σ * z2
            X1 = [a  0;
                  0  b]
            ...
            # weight by |ψ|^2 ~ exp(lp)
            s += local_kinetic(ans, a, b, c) + HB(X1, X2, X3, ν)
        end
        # normalize by partition (approximate) and samples
        return s / nsamples
    end
end
...
ν = @bind ν Slider(0.1:0.1:5.0, default=2.0, show_value=true) 

# ╔═╡ ba4264eb-66b1-4934-bf0b-de8ce42d162c
### Optimize r0 via simple gradient descent on estimated energy
begin

	# initialize the differentiable ansatz with r0=1, width (sigma)=0.5
    local ans = Ansatz(1.0, 0.5)

	# loss is the monte carlo sampled energy
    loss(a) = energy(a, ν; nsamples=2000)
	
    # gradient descent on two variational parameters
	η = 0.05 # learning rate
    for epoch in 1:100
        grads = Flux.gradient(loss, ans)[1]
        ans = Ansatz(
            ans.r0 - η * grads.r0,
            ans.σ  - η * grads.σ
        )
    ...
    println("Optimized r0 ≈ ", round(ans.r0, digits=3),", σ ≈ ", round(ans.σ, digits=3), ", E0 ≈ ", energy(Ansatz(ans.r0, ans.σ), ν))
end


# ╔═╡ 5b46a545-d8b1-48a8-9533-c01212629274
# begin
#     # define ranges
#     r_vals = range(0.5, 2.0, length=30)
#     σ_vals = range(0.1, 1.0, length=30)

#     # build a matrix E[i,j] = energy(r_vals[j], σ_vals[i])
#     E = [energy(Ansatz(r, s), ν; nsamples=500) for s in σ_vals, r in r_vals]

#     # heatmap: x→r₀, y→σ
#     heatmap(
#       r_vals, σ_vals, E;
#       xlabel="r₀", ylabel="σ",
#       title="⟨H⟩(r₀,σ) at ν=$(round(ν, digits=2))",
...
manifest_format = "2.0"
project_hash = "3b892960cad16e6969560038f136d50504c51995"
